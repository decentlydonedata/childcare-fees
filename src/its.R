...
for (d1 in fake_dates1) {
    df_placebo <- data %>%
      mutate(
        ...
        Placebo = ifelse(time >= d1, 1, 0),
        
        # fake post-trend (your time since structure)
        timepostPlacebo = pmax(time - d1, 0),
      )
    ...
    model <- plm(
      real_fee ~ Placebo + time + timepostPlacebo,
      data = pdata_placebo,
      model = "within"
    )
    
    placebo_results[[paste(d1)]] <- coeftest(model, vcov = vcovHC(model, type = "HC1", cluster = "group")) 
  }

placebo_results

summary(pdata$real_fee)

pdata_regular <- as.data.frame(pdata)

pdata_regular %>%
  group_by(treatment1, treatment2) %>%
  summarise(
    mean_fee = mean(real_fee, na.rm = TRUE),
    sd_fee   = sd(real_fee, na.rm = TRUE),
    min_fee = min(real_fee, na.rm = TRUE),
    max_fee = max(real_fee, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  )
